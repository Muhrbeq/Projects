/*
 * VEML7700.h
 *
 *  Created on: 31 Jan 2019
 *      Author: andreas.fredlund
 */

#ifndef VEML7700_H_
#define VEML7700_H_

#include "Global.h"

// extend nominal delay to ensure new sample is generated
#define EXTENDEN_DELAY(ms) HAL_Delay(2 * (ms))


#define VEML7700_TIMEOUT_RX_END		100						// Counts in transmit loop
#define VEML7700_TIMEOUT_TX_END		100						// Counts in receive loop

/* Function pointers */
typedef uint8_t (*GetRawCountsFunc)(uint16_t *raw_counts);




typedef enum
{
	ALS_GAIN_x1 = 0x0,    // x 1
	ALS_GAIN_x2 = 0x1,    // x 2
	ALS_GAIN_d8 = 0x2,    // x 1/8
	ALS_GAIN_d4 = 0x3 	  // x 1/4
} als_gain_t;

typedef enum
{
	ALS_INTEGRATION_25ms = 0xc,
	ALS_INTEGRATION_50ms = 0x8,
	ALS_INTEGRATION_100ms = 0x0,
	ALS_INTEGRATION_200ms = 0x1,
	ALS_INTEGRATION_400ms = 0x2,
	ALS_INTEGRATION_800ms = 0x3
} als_itime_t;

typedef enum
{
	ALS_PERSISTENCE_1 = 0x0,
	ALS_PERSISTENCE_2 = 0x1,
	ALS_PERSISTENCE_4 = 0x2,
	ALS_PERSISTENCE_8 = 0x3
} als_persist_t;

typedef enum
{
	ALS_POWER_MODE_1 = 0x0,
	ALS_POWER_MODE_2 = 0x1,
	ALS_POWER_MODE_3 = 0x2,
	ALS_POWER_MODE_4 = 0x3
} als_powmode_t;

enum
{
	STATUS_OK = 0,
	STATUS_ERROR = 0xff
};

enum
{
	VEML7700_ADDRESS = 0x10
};
enum
{
	COMMAND_ALS_SM = 0x00,
	ALS_SM_MASK = 0x1800,
	ALS_SM_SHIFT = 11
};
enum
{
	COMMAND_ALS_IT = 0x00,
	ALS_IT_MASK = 0x03c0,
	ALS_IT_SHIFT = 6
};
enum
{
	COMMAND_ALS_PERS = 0x00,
	ALS_PERS_MASK = 0x0030,
	ALS_PERS_SHIFT = 4
};
enum
{
	COMMAND_ALS_INT_EN = 0x00,
	ALS_INT_EN_MASK = 0x0002,
	ALS_INT_EN_SHIFT = 1
};
enum
{
	COMMAND_ALS_SD = 0x00,
	ALS_SD_MASK = 0x0001,
	ALS_SD_SHIFT = 0
};
enum
{
	COMMAND_ALS_WH = 0x01
};
enum
{
	COMMAND_ALS_WL = 0x02
};
enum
{
	COMMAND_PSM = 0x03,
	PSM_MASK = 0x0006,
	PSM_SHIFT = 1
};
enum
{
	COMMAND_PSM_EN = 0x03,
	PSM_EN_MASK = 0x0001,
	PSM_EN_SHIFT = 0
};
enum
{
	COMMAND_ALS = 0x04
};
enum
{
	COMMAND_WHITE = 0x05
};
enum
{
	COMMAND_ALS_IF_L = 0x06,
	ALS_IF_L_MASK = 0x8000,
	ALS_IF_L_SHIFT = 15
};
enum
{
	COMMAND_ALS_IF_H = 0x06,
	ALS_IF_H_MASK = 0x4000,
	ALS_IF_H_SHIFT = 14
};





// raw device commands
uint8_t VEML7700_SetGain(als_gain_t gain);
uint8_t VEML7700_GetGain(als_gain_t *gain);
uint8_t VEML7700_SetIntegrationTime(als_itime_t itime);
uint8_t VEML7700_GetIntegrationTime(als_itime_t *itime);
uint8_t VEML7700_SetPersistence(als_persist_t persist);
uint8_t VEML7700_SetPowerSavingMode(als_powmode_t powmode);
uint8_t VEML7700_SetPowerSaving(uint8_t enabled);
uint8_t VEML7700_SetInterrupts(uint8_t enabled);
uint8_t VEML7700_SetPower(uint8_t on);
uint8_t VEML7700_SetALSHighThreshold(uint16_t thresh);
uint8_t VEML7700_SetALSLowThreshold(uint16_t thresh);
uint8_t VEML7700_GetALS(uint16_t *als);
uint8_t VEML7700_GetWhite(uint16_t *white);
uint8_t VEML7700_GetHighThresholdEvent(uint8_t *event);
uint8_t VEML7700_GetLowThresholdEvent(uint8_t *event);

// higher-level API
uint8_t VEML7700_GetALSLux(float *lux);
uint8_t VEML7700_GetWhiteLux(float *lux);
uint8_t VEML7700_GetAutoALSLux(float *lux);
uint8_t VEML7700_GetAutoWhiteLux(float *lux);
//uint8_t VEML7700_GetAutoALSLux(float *lux,
//					als_gain_t *auto_gain,
//					als_itime_t *auto_itime,
//					uint16_t *raw_counts);
//
//uint8_t VEML7700_GetAutoWhiteLux(float *lux,
//					  als_gain_t *auto_gain,
//					  als_itime_t *auto_itime,
//					  uint16_t *raw_counts);

uint8_t VEML7700_SampleDelay(void);
void VEML7700_ScaleLux(uint16_t raw_counts, float *lux);
uint8_t VEML7700_GetAutoXLux(float *lux, GetRawCountsFunc counts_func, als_gain_t *auto_gain,
		als_itime_t *auto_itime, uint16_t *raw_counts);

void VEML7700_Initialize(void);





#endif /* VEML7700_H_ */
