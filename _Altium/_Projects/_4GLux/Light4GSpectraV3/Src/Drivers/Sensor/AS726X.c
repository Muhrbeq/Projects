...
void AS726X_takeMeasurementsRaw(uint8_t gain, Spectrum *sp)
{
	if (AS726X_begin(gain, AS726X_MODE_ONE_SHOT) != HAL_OK) //Gain and One-shot mode
	{
		ErrorAddLog(LOG_FUNC_AS726X, LOG_SUB_MEASURE_RAW, LOG_TYPE_READ, LOG_STATUS_FAIL, 0, 0, 0, GetErrorStruct());
	}
...
uint16_t AS726X_get_Normalized_Value(uint32_t intensity, uint8_t gain)
{
	...
	{
		ErrorAddLog(LOG_FUNC_AS726X, LOG_SUB_NORM, LOG_TYPE_NONE, LOG_STATUS_MAX, AS726X_INTEGRATION_TIME, gain, 0, GetErrorStruct());
	}
